...
const Post = (props) => {
    ...
    const handleLoadComments = () => getComments({variables: {postId: post.id}});

    React.useEffect(() => {console.log("post rendered", post.id)});
    
    return (
    <div className="post-entity" data-post-id={post.id}>
                ...
                <div className="media-content">
                    <div className="content">
                        <p>
                            ...
                            <small>
                                <i className="far fa-thumbs-up social-action" style={{color: likers.includes(state.user.id) ? "green" : "initial"}} onClick={() => handleVote(post.id, 1)}> {likers.length > 0 && likers.length}</i> 
                                <i className="far fa-thumbs-down social-action" style={{color: dislikers.includes(state.user.id) ? "red" : "initial"}} onClick={() => handleVote(post.id, -1)}> {dislikers.length > 0 && dislikers.length}</i> 
                    ...
                    {commentsData && <Comments comments={commentsData.post.comments} />}
                </div>
            </article>
            {commentsData && <WriteComment commentContent={commentContent} setCommentContent={handleSetCommentContent} postComment={handlePostComment} />}
        </div>
    );
};
